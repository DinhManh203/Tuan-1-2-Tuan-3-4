...
<body>
    ...
    <script>
        ...
        function deleteRole(id, name, description) {
            roleToDelete = { id: id, name: name, description: description }; // Lưu thông tin vai trò cần xóa
        ...
        function confirmDelete() {
            const { id, name, description } = roleToDelete;
            var xhr = new XMLHttpRequest();
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    closeConfirmPopup();
                    showNotification("Delete role successful!", name, description);
                    setTimeout(() => {
                        location.reload(); // Tải lại trang để cập nhật danh sách vai trò
                    }, 2000); // Delay 2 giây
                }
